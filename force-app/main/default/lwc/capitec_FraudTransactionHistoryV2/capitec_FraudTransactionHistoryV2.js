...
import {calculateTotalFlaggedTransactionValueAndUpdateCase, transactionHistoryGetAccs,formatDate, getStartDate, getTransactionHistory, postApplyActions, postChase, fetchConsultantCPByCaseId, preparePopupsContent} from 'c/capitec_FraudTHDataHandler'
...
export default class CapitecFraudCaseActions extends LightningElement {
    ...
    @track accType = ''; // Default value
    accountTypeOptions = [
        { label: 'All', value: 'All' },
        { label: 'Card Authorisation', value: 'CARD_AUTHORISATION' },
        { label: 'Card Authorisation Financial', value: 'CARD_AUTH_FINANCIAL' },
    ...
    async handleGetTransactions(event){
        ...
        let accTypeVal = this.template.querySelector('[data-id="accType-fin-id"]').value;
    ...
    async handleApplyActions(event){
        ...
        let itemsToFraudHoldChaseFlag = {transactions : [], caseId : this.recordId, consultantIdentifier : this.consultantCP}; // this should link to the consultant CP number

        this.itemsToDisplay.forEach(item => {
            if((item.flaggedFraud !== item.newFraudFlagged) || (item.flaggedHold !== item.newHoldFlagged) || (item.flaggedChase !== item.newChaseFlagged)){ //FFSF-407 - Chase Flag added to the condition
    ...
    async handlePostChased(event){ //FFSF-407 - Chase Flag moved to fire under the Apply Actions Button
        ...
        this.showSpinner = true;
        let itemsToChaseFlag = {transactions : [], caseId : this.recordId, consultantIdentifier : this.consultantCP};

        this.itemsToDisplay.forEach(item => {
            if(item.flaggedChase !== item.newChaseFlagged){
    ...
    async performGetCallouts(calloutAccountNumbers, caseId, startDateVal, endDateVal, accTypeVal){
        let promises = calloutAccountNumbers.map(accNumber => {
            return getTransactionHistory(accNumber, caseId, startDateVal, endDateVal, accTypeVal)
                .then(result => {
                    try{
