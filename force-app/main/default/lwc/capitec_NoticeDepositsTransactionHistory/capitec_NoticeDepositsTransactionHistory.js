 ...
 * Last modified by Cornelia Smit on 25.07.2024 for CECSF-195
 */

import {api, LightningElement, track} from 'lwc';
import {transactionHistoryGetAccs,formatDate, getStartDate, getTransactionHistory, postApplyActions, postChase, fetchConsultantCPByCaseId, preparePopupsContent} from 'c/capitec_NoticeDepositsTHDataHandler'
import LightningAlert from "lightning/alert";
import { updateRecord } from 'lightning/uiRecordApi'; 
import gethandleFlagToCase from '@salesforce/apex/Capitec_NoticeDepositsTHController.gethandleFlagToCase';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';


export default class CapitecNoticeDepositsTransactionHistory extends LightningElement {
    ...
    @track noRecordsMessage = ''; //CECSF-195
    @track noAccountOptions = false; //CECSF-195
    ...
    async handleGetTransactions(event){
        ...
        //console.log(JSON.stringify(this.itemsToDisplay));
        //console.log(this.itemsToDisplay.length);
        //CECSF-195 added message for no transactions and disabled flagging button
        if (this.itemsToDisplay.length === 0) {
            this.noRecordsMessage = 'No transactions exist for the selected timeframe';
            ...
            try{
                if (JSON.stringify(result) === "204" || JSON.stringify(result) === "200"){
                    ...
                    LightningAlert.open({
                        ...
                        theme:'warning',
                        label: 'One or more actions did not succeed. Please try again later',
                    });
                    this.handleGetTransactions('');
                }
            }
    ...
    async handleInitiateData(recordId){
        ...
        await transactionHistoryGetAccs(recordId).then(result =>{
            try{
                ...
                this.noAccountOptions = result.length < 1 || result[0].label == 'No accounts available'; //CECSF-195 disable button if no accounts available
            } catch (e){
                console.log(JSON.stringify(e));
                this.noAccountOptions = true; //CECSF-195
            }
            // console.log(JSON.stringify(result));
        });
    }
            ...
            return getTransactionHistory(accNumber, caseId, startDateVal, endDateVal)
                .then(result => {
                    try {
                        ...
                        result = result.filter(currentItem => currentItem.transactionDate); //CECSF-195 filter out non-data
                        result.forEach(currentItem => {
                            if (currentItem.transactionDate) {
                                // Convert the date format
                                let date = new Date(currentItem.transactionDate);
                                ...
                                currentItem.formattedTransactionDate = formattedDate; //CECSF-280 format date for display only
                            }
                        });
...
async handleFlagToCase(event) {
    ...
    try {
        var result = await gethandleFlagToCase({ caseId: this.recordId, selectedItems: selectedItems });
